...
CREATE TABLE IF NOT EXISTS contractor_activity_log (
  ...
  event_type VARCHAR(50) NOT NULL,  -- 'login', 'wallet_add', 'lead_click', 'lead_purchase', 'profile_update'
...
CREATE TABLE IF NOT EXISTS sms_send_log (
  ...
  cost_cents INTEGER DEFAULT 79,  -- Cost in cents (default $0.0079)
  timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  status VARCHAR(50) DEFAULT 'sent',  -- 'sent', 'delivered', 'failed', 'skipped', 'blocked_by_limit'
...
CREATE INDEX IF NOT EXISTS idx_contractor_activity_log_event_type ON contractor_activity_log(event_type);
...
CREATE INDEX IF NOT EXISTS idx_sms_send_log_lead_id ON sms_send_log(lead_id);
...
CREATE INDEX IF NOT EXISTS idx_sms_send_log_category ON sms_send_log(category);
CREATE INDEX IF NOT EXISTS idx_sms_send_log_location ON sms_send_log(location);
...
CREATE POLICY "Allow anonymous read" ON contractor_activity_log FOR SELECT USING (true);
CREATE POLICY "Allow anonymous read" ON sms_send_log FOR SELECT USING (true);
CREATE POLICY "Allow anonymous read" ON sms_config FOR SELECT USING (true);
