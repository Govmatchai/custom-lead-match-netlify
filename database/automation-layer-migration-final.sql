ALTER TABLE leads ADD COLUMN IF NOT EXISTS validation_email_status VARCHAR(20) DEFAULT 'pending';
ALTER TABLE leads ADD COLUMN IF NOT EXISTS validation_phone_status VARCHAR(20) DEFAULT 'pending';
ALTER TABLE leads ADD COLUMN IF NOT EXISTS validation_zip_status VARCHAR(20) DEFAULT 'pending';

DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'leads_validation_email_status_check') THEN
    ALTER TABLE leads ADD CONSTRAINT leads_validation_email_status_check 
    CHECK (validation_email_status IN ('pending', 'valid', 'invalid', 'error', 'skipped', 'unknown'));
  END IF;
END $$;

DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'leads_validation_phone_status_check') THEN
    ALTER TABLE leads ADD CONSTRAINT leads_validation_phone_status_check 
    CHECK (validation_phone_status IN ('pending', 'valid', 'invalid', 'error', 'skipped', 'unknown'));
  END IF;
END $$;

DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'leads_validation_zip_status_check') THEN
    ALTER TABLE leads ADD CONSTRAINT leads_validation_zip_status_check 
    CHECK (validation_zip_status IN ('pending', 'valid', 'invalid', 'error', 'skipped', 'unknown'));
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Allow all for service role" ON refund_requests;
DROP POLICY IF EXISTS "Allow anonymous read" ON refund_requests;
...
DROP POLICY IF EXISTS "Allow all for service role" ON weekly_reports;
DROP POLICY IF EXISTS "Allow anonymous read" ON weekly_reports;
